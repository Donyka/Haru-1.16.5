...
import kz.haru.api.via.model.ComparableProtocolVersion;
import kz.haru.api.via.model.Platform;
import kz.haru.api.via.platform.ViaBackwardsPlatformImpl;
import kz.haru.api.via.platform.ViaVersionPlatformImpl;
import kz.haru.api.via.platform.viaversion.ViaInjector;
import kz.haru.api.via.platform.viaversion.ViaProviders;
import kz.haru.api.via.util.JLoggerToLog4j;
...
public class ViaLoadingBase {
	...
	public static final Logger LOGGER = new JLoggerToLog4j((org.apache.logging.log4j.core.Logger) LogManager.getLogger("ViaVersion"));
	...
	public static final Platform PLATFORM_VIA_BACKWARDS = new Platform("ViaBackwards",
			() -> ViaLoadingBase.inClassPath("com.viaversion.viabackwards.api.ViaBackwardsPlatform"),
	...
	public ViaLoadingBase(LinkedList<Platform> platforms, File runDirectory, int nativeVersion,
			BooleanSupplier forceNativeVersionCondition, Supplier<JsonObject> dumpSupplier,
			Consumer<com.viaversion.viaversion.api.platform.providers.ViaProviders> providers,
			Consumer<ViaManagerImpl.ViaManagerBuilder> managerBuilderConsumer,
			Consumer<ComparableProtocolVersion> onProtocolReload) {
		this.platforms = platforms;
		this.runDirectory = new File(runDirectory, "esckere");
	...
	public void initPlatform() {
		...
		for (ProtocolVersion preProtocol : Platform.TEMP_INPUT_PROTOCOLS) {
			PROTOCOLS.put(preProtocol, new ComparableProtocolVersion(preProtocol.getVersion(), preProtocol.getName(),
					Platform.TEMP_INPUT_PROTOCOLS.indexOf(preProtocol)));
		}
		this.targetProtocolVersion = this.nativeProtocolVersion = ViaLoadingBase
				.fromProtocolVersion(ProtocolVersion.getProtocol(this.nativeVersion));
		ViaVersionPlatformImpl viaVersionPlatform = new ViaVersionPlatformImpl(LOGGER);
		ViaManagerImpl.ViaManagerBuilder builder = ViaManagerImpl.builder().platform(viaVersionPlatform)
				.loader(new ViaProviders()).injector(new ViaInjector());
	...
	public static ComparableProtocolVersion fromProtocolId(int protocolId) {
		return PROTOCOLS.values().stream().filter(protocol -> protocol.getVersion() == protocolId).findFirst()
				.orElse(null);
	}
	...
	public static class ViaLoadingBaseBuilder {
		...
		public ViaLoadingBaseBuilder providers(
				Consumer<com.viaversion.viaversion.api.platform.providers.ViaProviders> providers) {
			this.providers = providers;
			return this;
		}

		public ViaLoadingBaseBuilder managerBuilderConsumer(
				Consumer<ViaManagerImpl.ViaManagerBuilder> managerBuilderConsumer) {
			this.managerBuilderConsumer = managerBuilderConsumer;
			return this;
		}
		...
		public void build() {
			...
			new ViaLoadingBase(this.platforms, this.runDirectory, this.nativeVersion, this.forceNativeVersionCondition,
					this.dumpSupplier, this.providers, this.managerBuilderConsumer, this.onProtocolReload);
		}
	}
}

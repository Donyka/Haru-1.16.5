package kz.haru.api.via.netty.handler;
...
public class CommonTransformer {
	public static void decompress(ChannelHandlerContext ctx, ByteBuf buf) throws InvocationTargetException {
		...
		ByteBuf decompressed = handler instanceof MessageToMessageDecoder
				? (ByteBuf) PipelineUtil.callDecode((MessageToMessageDecoder) handler, ctx, (Object) buf).get(0)
				: (ByteBuf) PipelineUtil.callDecode((ByteToMessageDecoder) handler, ctx, (Object) buf).get(0);
		try {

...
public class MouseHelper
{
    ...
    private void mouseButtonCallback(long handle, int button, int action, int mods)
    {
        if (handle == this.minecraft.getMainWindow().getHandle())
        {
            ...
            Haru.getInstance().getEventManager().callEvent(new ButtonInputEvent(action, button));
            if (action == 0) {
                if (this.minecraft.currentScreen instanceof ChatScreen) {
                    for (Draggable draggable : DraggableManager.getDraggables().values()) {
            ...
            if (flag)
            {
                ...
                float mouseX = (float) (this.mouseX * (double)this.minecraft.getMainWindow().getScaledWidth() / (double)this.minecraft.getMainWindow().getWidth());
                float mouseY = (float) (this.mouseY * (double)this.minecraft.getMainWindow().getScaledHeight() / (double)this.minecraft.getMainWindow().getHeight());
                ...
                for (ElementDraggable elementDraggable : InterfaceModule.get().getElementDraggables()) {
                    if (elementDraggable.getDraggable().getModule().isEnabled()) {
                        elementDraggable.mouseClicked(mouseX, mouseY, button);
                    }
                }

                KeyboardFunctions.onKeyPress(button);
        ...
        if (handle == Minecraft.getInstance().getMainWindow().getHandle())
        {
            ...
            if (this.minecraft.loadingGui == null)
            {
                ...
                else if (this.minecraft.player != null)
                {
                    ...
                    this.accumulatedScrollDelta -= f1;
    ...
    public void updatePlayerLook()
    {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused())
        {
            ...
            FreeCameraModule freeCameraModule = kz.haru.implement.modules.player.FreeCameraModule.get();

            if (freeCameraModule.isEnabled() && freeCameraModule.fakePlayer != null) {
                freeCameraModule.fakePlayer.rotateTowards(d2, d3 * (double)i);
            } else {
     ...
     * currently displayed
     */
    public void grabMouse(boolean closeScreen)
    {
        if (this.minecraft.isGameFocused())
        {
            if (!this.mouseGrabbed)
            {
                ...
                if (closeScreen) this.minecraft.displayGuiScreen(null);

...
public class CommandSuggestionHelper
{
    ...
    public void init()
    {
        ...
        } else if (stringreader.canRead() && stringreader.peek() == Haru.getInstance().getCommandManager().getCommandPrefix().charAt(0)) {
            stringreader.skip();
            CommandDispatcher<ISuggestionProvider> commanddispatcher = Haru.getInstance().getCommandManager().getCommandDispatcher();

            if (this.parseResults == null) {
                this.parseResults = commanddispatcher.parse(stringreader, null);
            }
            ...
            if (event.completions != null) {
                ...
                if (event.completions.length == 0) {
                    ...
                    List<Suggestion> suggestionList = Stream.of(event.completions)
                            .map(s1 -> new Suggestion(range, s1))

...
import kz.haru.api.waveycapes.interfaces.CapeHolder;
import kz.haru.api.waveycapes.sim.StickSimulation;
...
public abstract class AbstractClientPlayerEntity extends PlayerEntity implements CapeHolder
{
    ...
    private final ResourceLocation CAPE_LOCATION = new ResourceLocation(ClientInfo.clientName.toLowerCase() + "/images/cape/haru.png");

    @Nullable
    public ResourceLocation getLocationCape() {
        if (this.hasCustomCape()) {

            //if (this.hasCustomCape() && !NoRender.element.is("Плащ").getValue() && (Haruka.getInst().getModuleManager().getCosmetics().effects.is("Custom Models").getValue() &&  !Haruka.getInst().getModuleManager().getCosmetics().onyskin.getValue() && Haruka.getInst().getModuleManager().getCosmetics().cape.getValue())) {
    ...
    public boolean hasCustomCape() {
        return this instanceof ClientPlayerEntity
                || FriendManager.isFriend(this.getNameClear());
    }

...
import net.baritone.api.BaritoneAPI;
import net.baritone.api.IBaritone;
import net.baritone.api.event.events.ChatEvent;
import net.baritone.api.event.events.PlayerUpdateEvent;
import net.baritone.api.event.events.type.EventState;
...
{
    public float packetYaw, packetPitch;
    ...
    public void tick()
    {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ())))
        {
            ...
            Haru.getInstance().getEventManager().callEvent(new UpdateEvent());

            if (this.isPassenger())
            {
                float[] angles = new float[]{this.rotationYaw, this.rotationPitch};
                if (RotationController.getInstance().getCurrentRotationPlan() != null) {
                    Rotation sex = RotationController.getInstance().getRotation();
                    angles = new float[]{sex.yaw, sex.pitch};
                }
                this.connection.sendPacket(new CPlayerPacket.RotationPacket(angles[0], angles[1], this.onGround));
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        {
            MovingEvent event = new MovingEvent();
            Haru.getInstance().getEventManager().callEvent(event);

            Rotation rotation = RotationController.getInstance().getRotation();

            if (RotationController.getInstance().getCurrentRotationPlan() != null) {
                this.renderYawOffset = rotation.yaw;
                this.rotationYawHead = rotation.yaw;
            }

            double currentX = this.getPosX();
            double currentY = this.getPosY();
            double currentZ = this.getPosZ();
            double currentYaw = rotation.yaw;
            double currentPitch = rotation.pitch;

            if (event.isCancelled()) {
                return;
            }

            double d4 = currentX - this.lastReportedPosX;
            double d0 = currentY - this.lastReportedPosY;
            double d1 = currentZ - this.lastReportedPosZ;
            double d2 = currentYaw - this.lastReportedYaw;
            double d3 = currentPitch - this.lastReportedPitch;
            ...
            {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(currentX, currentY, currentZ, (float) currentYaw, (float) currentPitch, this.onGround));
            }
            else if (flag1)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionPacket(currentX, currentY, currentZ, this.onGround));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.RotationPacket((float) currentYaw, (float) currentPitch, this.onGround));
            }
            ...
            if (flag1)
            {
                this.lastReportedPosX = currentX;
                this.lastReportedPosY = currentY;
                this.lastReportedPosZ = currentZ;
            ...
            if (flag2)
            {
                ...
                this.lastReportedPitch = (float) currentPitch;
            }
    ...
    {
        HChatEvent event = new HChatEvent(message);
        Haru.getInstance().getEventManager().callEvent(event);
        ...
        if (Haru.baritoneAviable()) {
            ChatEvent event1 = new ChatEvent(message);
            IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) this);
            if (baritone == null) {
                return;
            }
            baritone.getGameEventHandler().onSendChatMessage(event1);
    ...
    public void closeScreen() {
        CloseScreenEvent event = new CloseScreenEvent(this.mc.currentScreen, this.openContainer.windowId);
        Haru.getInstance().getEventManager().callEvent(event);
    ...
    {
        PushingEvent event = new PushingEvent(PushingEvent.PushingType.BLOCK);
        Haru.getInstance().getEventManager().callEvent(event);
    ...
    public void livingTick()
    {
        ...
        {
            SlowingEvent event = new SlowingEvent();
            Haru.getInstance().getEventManager().callEvent(event);
        ...
        if (SprintModule.get().isCanSprint()) {
            ...
            if (SprintModule.get().isEnabled() && !this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && (!this.isPotionActive(Effects.BLINDNESS))) {
    ...
    protected void updateAutoJump(float movementX, float movementZ)
    {
        if (this.canAutoJump())
        {
            ...
            if (f1 <= 0.001F)
            {
                ...
                if (RotationController.getInstance().getCurrentRotationPlan() != null) {
                    if (RotationController.getInstance().getCurrentRotationPlan().isMoveCorrection()) {
                        yaw = RotationController.getInstance().getRotation().yaw;
                    }
                }
                ...
                float f4 = MathHelper.sin(yaw * ((float)Math.PI / 180F));
                float f5 = MathHelper.cos(yaw * ((float)Math.PI / 180F));

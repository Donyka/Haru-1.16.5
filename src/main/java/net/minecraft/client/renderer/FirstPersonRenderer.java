...
import kz.haru.implement.modules.render.ViewModelModule;
...
public class FirstPersonRenderer
{
    ...
    {
        SwingAnimationModule swing = SwingAnimationModule.get();
        float power = swing.isEnabled() ? swing.power.getValue() * 10 : 80f;
    ...
    {
        SwingAnimationModule swingAnim = SwingAnimationModule.get();
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        matrixStackIn.translate((float) i * 0.56F, -0.52F + equippedProg * (swingAnim.isEnabled() ? 0.0F : -0.6F), -0.72F);
    }
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn))
        {
            ...
            else
            {
                ...
                ViewModelModule viewModel = ViewModelModule.get();

                if (viewModel.isEnabled()) {
                    if (flag3) {
                        matrixStackIn.translate(viewModel.rightX.getValue(),
                                viewModel.rightY.getValue(), viewModel.rightZ.getValue());
                    } else {
                        matrixStackIn.translate(viewModel.leftX.getValue(),
                                viewModel.leftY.getValue(), viewModel.leftZ.getValue());
                    }
                }
                ...
                else
                {
                    ...
                    boolean workWorkAndSexWithMeYesYesDaddy = swingAnimation.isEnabled() && handIn.equals(Hand.MAIN_HAND) && (!itemStackMainHand.getItem().equals(Items.SHIELD));
                    ...
                    if (workWorkAndSexWithMeYesYesDaddy) {

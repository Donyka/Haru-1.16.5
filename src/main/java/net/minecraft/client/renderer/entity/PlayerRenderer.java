...
import kz.haru.api.waveycapes.render.CustomCapeRenderLayer;
...
public class PlayerRenderer extends LivingRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>
{
    ...
    protected void applyRotations(AbstractClientPlayerEntity entityLiving, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks)
    {
        ...
        if (entityLiving.isElytraFlying())
        {
            ...
            {
                float pit = RotationController.getInstance().getCurrentRotationPlan() == null || entityLiving != Minecraft.getInstance().player ? entityLiving.rotationPitch : RotationController.getInstance().getRotation().pitch;

                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(f2 * (-90.0F - pit)));
            }
        ...
        else if (f > 0.0F)
        {
            ...
            super.applyRotations(entityLiving, matrixStackIn, ageInTicks, rotationYaw, partialTicks);
            float f3 = entityLiving.isInWater() ? -90.0F - pit : -90.0F;

...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M>
{
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn))
        {
            ...
            float prev = entityIn.prevRotationPitch;
            float cur = entityIn.rotationPitch;

            Rotation rot = RotationController.getInstance().getRotation();
            Rotation prevRot = RotationController.getInstance().getPreviousRotation();

            if (RotationController.getInstance().getCurrentRotationPlan() != null && entityIn == Minecraft.getInstance().player) {
                cur = rot.pitch;
                prev = prevRot.pitch;
            }

            float f7 = MathHelper.lerp(partialTicks, prev, cur);
            //float f7 = (entityIn instanceof PlayerEntity && entityIn == Minecraft.getInstance().player) ? entityIn.prevRotationPitchHead + (entityIn.rotationPitchHead - entityIn.prevRotationPitchHead) * partialTicks : entityIn.prevRotationPitch + (entityIn.rotationPitch - entityIn.prevRotationPitch) * partialTicks;

...
import net.baritone.utils.accessor.IPlayerControllerMP;
...
public class PlayerController implements IMinecraft, IPlayerControllerMP
{
    ...
    public ActionResultType processRightClick(PlayerEntity player, World worldIn, Hand hand)
    {
        ...
        else
        {
            ...
            if (ViaLoadingBase.getInstance().getTargetVersion().isNewerThanOrEqualTo(ProtocolVersion.v1_17)) {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(player.getPosX(), player.getPosY(), player.getPosZ(), player.rotationYaw, player.rotationPitch, player.isOnGround()));
            }

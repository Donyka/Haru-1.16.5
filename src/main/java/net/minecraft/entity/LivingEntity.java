...
import kz.haru.implement.modules.render.NameTagsModule;
import kz.haru.implement.modules.render.SwingAnimationModule;
...
public abstract class LivingEntity extends Entity
{
    ...
    {
        SwingAnimationModule swingAnimation = SwingAnimationModule.get();
        
        if (swingAnimation.isEnabled() && this == Minecraft.getInstance().player) {
            return 25 - (swingAnimation.speed.getValue().intValue() * 2);
        }
    ...
    {
        if (NameTagsModule.get().isEnabled()) {
            return false;
        }
    ...
    protected void jump()
    {
        ...
        if (this instanceof ClientPlayerEntity) {
            if (Haru.baritoneAviable()) {
                IBaritone baritone1 = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) this);
                if (baritone1 != null) {
                    this.jumpRotationEvent = new RotationMoveEvent(RotationMoveEvent.Type.JUMP, this.rotationYaw);
                    baritone1.getGameEventHandler().onPlayerRotationMove(this.jumpRotationEvent);
        ...
        if (this.isSprinting())
        {
            ...
            float f1 = (currentYaw) * ((float) Math.PI / 180F);
    ...
    public void travel(Vector3d travelVector)
    {
        if (this.isServerWorld() || this.canPassengerSteer())
        {
            ...
            else if (this.isElytraFlying())
            {
                ...
                float rotatePitch = RotationController.getInstance().getCurrentRotationPlan() == null || this != Minecraft.getInstance().player ? this.rotationPitch : rotation.pitch;

                Vector3d vector3d1 = this.getLookVec();
                float f = rotatePitch * ((float)Math.PI / 180F);
    ...
    {
        PushingEvent event = new PushingEvent(PushingEvent.PushingType.ENTITY);

...
import net.baritone.api.event.events.RotationMoveEvent;
...
public abstract class Entity implements INameable, ICommandSource
{
    ...
    private Vector3d getAllowedMovement(Vector3d vec)
    {
        ...
        {
            Vector3d vector3d1 = collideBoundingBoxHeuristically(this, new Vector3d(vec.x, this.stepHeight, vec.z), axisalignedbb, this.world, iselectioncontext, reuseablestream);
            Vector3d vector3d2 = collideBoundingBoxHeuristically(this, new Vector3d(0.0D, this.stepHeight, 0.0D), axisalignedbb.expand(vec.x, 0.0D, vec.z), this.world, iselectioncontext, reuseablestream);
    ...
    public void moveRelative(float p_213309_1_, Vector3d relative)
    {
        ...
        kz.haru.common.utils.aiming.rotation.controller.Rotation rot = RotationController.getInstance().getRotation();
        RotationPlan rotPlan = RotationController.getInstance().getCurrentRotationPlan();

            if (rotPlan != null) {
                if (rotPlan.isMoveCorrection() && this == Minecraft.getInstance().player) {

                    if (Haru.baritoneAviable()) {
                        if (BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) this) == null) {
                            return;
                        }

                        IBaritone baritone1 = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) mc.player);

                        if (baritone1.getMineProcess().isActive() || baritone1.getFollowProcess().isActive() || baritone1.getBuilderProcess().isActive() || baritone1.getFollowProcess().isActive() || baritone1.getExploreProcess().isActive() || baritone1.getFarmProcess().isActive() || baritone1.getCustomGoalProcess().isActive() || baritone1.getGetToBlockProcess().isActive()) {
                            ...
                            yaw = net.baritone.api.utils.Rotation.YAW;
                        }
                    }
    ...
    {
        double d0 = MathHelper.lerp(p_242282_1_, this.prevPosX, this.getPosX());
        double d1 = MathHelper.lerp(p_242282_1_, this.prevPosY, this.getPosY());
        double d2 = MathHelper.lerp(p_242282_1_, this.prevPosZ, this.getPosZ());
        return new Vector3d(d0, d1, d2);
    }
    ...
    {
        kz.haru.common.utils.aiming.rotation.controller.Rotation rot = RotationController.getInstance().getCurrentRotation();

        return this.getVectorForRotation(rot == null ? rotationPitch : rot.pitch, rot == null ? rotationYaw : rot.yaw);
    }
    ...
    {
        if (kz.haru.implement.modules.render.NameTagsModule.get().isEnabled()) {
            return false;
        }

    ...
    public void tick()
    {
        ...
        if (this.isAttachedToEntity())
        {
            ...
            if (this.boostedEntity != null)
            {
                ...
                {
                    FireworkEvent event = new FireworkEvent(1.5f, boostedEntity);
                    Haru.getInstance().getEventManager().callEvent(event);

                    Vector3d vector3d = this.boostedEntity.getLookVec();

                    float boosting = event.getSpeed();
                    Vector3d vector3d1 = this.boostedEntity.getMotion();
                    this.boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1D + (vector3d.x * boosting - vector3d1.x) * 0.5D, vector3d.y * 0.1D + (vector3d.y * boosting - vector3d1.y) * 0.5D, vector3d.z * 0.1D + (vector3d.z * boosting - vector3d1.z) * 0.5D));
                }
